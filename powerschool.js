...
let powerschool = async (username, password, shallow, screenshots, headless, outputDirectory) => {
  // INIT
  ...
  // WRAP UP
  browser.close();
  fileSystem.writeFile('output/export.json', JSON.stringify(result), err => {  
    if (err) throw err;
    console.log('Done!');
  });
};
...
program
  ...
  .action(options => {
    powerschool(options.username, options.password, options.shallow, options.screenshots, options.headless, 'output');
  });
